...
def add_guest_entry():
    ...
    if not session.get('logged_in'):
        pass  # unauthorised?
    db = get_db()
    db.execute('insert into guest_entries (title, text) values (?, ?)',
                 [request.form['title'], request.form['text']])
    db.commit()
    flash('New entry was successfully posted')
...
def search_entries():
    ...
    entries = cur.fetchall()
    return render_template('search_entries.html', entries=entries)

"""

- find a specific blog entry
...
- search for blog entries containing a certain word
- showing 20 blog entries per page
